...
class Solution {
    ...
    public ListNode reverseKGroup(ListNode head, int k) {
        ...
        ListNode preGTail = null;// 上组翻转后的尾节点
        ListNode nextGHead = head;// 下组头结点
        while (nextGHead != null) {
            // k个一组
            ListNode ghead = nextGHead;
            ListNode gtail = ghead;
            for (int i = 1; i < k && gtail != null; i++) {
                gtail = gtail.next;
            }
            if (gtail == null) break;// 说明最后几个无法组成一组，不翻转了
            nextGHead = gtail.next;// 记录下一组头结点

            // 组内翻转
            reversePart(ghead, gtail);
            if (result == null) result = gtail;// 第一个翻转数组头结点

            // 上组末尾连这组
            if (preGTail != null) preGTail.next = gtail;
            preGTail = ghead;

            // 这组末尾连下组
            ghead.next = nextGHead;
        }
        return result == null ? head : result;// result为null说明一组都没反转过
    }

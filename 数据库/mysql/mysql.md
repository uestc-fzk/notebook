...
- 写3000~5000
- 单实例读6000~8000，集群：实例数*单机读qps

![mysql部署架构](mysql.assets/mysql部署架构.jpg)

- 2，1主1备主，备主库不可读，仅用于故障迁移和恢复场景。
- 2N，N是单机房可承接整个集群3倍读qps的从库数量，双机房部署
- 2M，M是离线用途实例，如跑批、监控、DTS(数据变更获取服务)等
...
1. 性能区别：
   - MyISAM类型强调局限场景下性能，单线程只插入或只查询比Innodb快，因为MyISAM的表锁开销小于Innodb的行锁开销。
   - MyISAM实现简单，在读取到索引后直接操作磁盘，而Innodb需维护bufferpool。但如果数据比较离散且操作系统cache中没有数据，MyISAM会大量随机IO，此时速度未必比Innodb快。
   - MyISAM只有表锁，Innodb支持行锁，在并发混合读写时，Innodb快的多。
2. Innodb支持事务
3. 可靠性：Innodb表有redo日志用于故障恢复，MyISAM直接采用文件系统存储，不具备恢复能力，在大批量数据灌入情况下易损坏。
...
二级索引就类似于一个新表，其数据对应的是主键id，索引越多，更新数据代价越大(每多一个索引相当于多操作一个表)：
...
禁用分区表：逻辑上一个表，物理上多个文件，存在跨区访问问题，且DDL操作会同时对所有分区表进行，压力太大。不如直接物理分表。

分表名格式为`表名_编号`，如t_user_0，分表数量为2的幂。若以时间分表，如按月分的统计表可以取名`statistics_202410`。

尽量避免宽表，冷热数据分离到多个表，如订单表和订单详情表。

字段类型越小越好，主要是索引越小，单页索引结点越多，I/O少。

字段设置not null和default值，因null要单独处理，且更占空间(?)。



DML控制事务处理间隔和大小，每1000条可以sleep1秒。

数据库账号权限遵循权限最小原则，只读账号以`_r`结尾，读写账号以`_w`结尾，监控账号为`monitor`。

应用程序连接数据库，必须配置失败重连机制，最小和最大连接数，连接超时(最大1s)，连接回收机制(空闲超时最大30s)。

应用程序必须使用预编译语句进行crud，预编译语句可一次解析重复使用，减少sql编译时间，且可有效解决sql注入问题。

...
当下的应用是 SQL 和 NoSQL 一起使用，技术没有高低之分.
...
![image-20210831211013456](https://uestc-fzk.gitee.io/notebook/数据库/redis/redis.assets/image-20210831211013456.png)
...
> 由于当前的网络硬件肯定会出现延迟丢包等问题，所以**分区容错性是我们必须需要实现**的。
>
...
**CAP理论的核心**是：一个分布式系统不可能同时很好的满足一致性，可用性和分区容错性这三个需求，
...
完全开源、C编写、遵守 BSD 协议、高性能的 key-value 数据库。
...
![image-20210831213659135](https://uestc-fzk.gitee.io/notebook/数据库/redis/redis.assets/image-20210831213659135.png)
...
![image-20210831231349315](https://uestc-fzk.gitee.io/notebook/数据库/redis/redis.assets/image-20210831231349315.png)
...
我们首先要明白，Redis很快！官方表示，因为Redis是**基于内存**的操作，**CPU不是Redis的瓶颈**，Redis的**瓶颈最有可能是机器内存**的大小或者网络带宽。既然单线程容易实现，而且CPU不会成为瓶颈，那就顺理成章地采用单线程的方案了！
...
![image-20210903114842004](https://uestc-fzk.gitee.io/notebook/数据库/redis/redis.assets/image-20210903114842004.png)
...
![image-20210901153120391](https://uestc-fzk.gitee.io/notebook/数据库/redis/redis.assets/image-20210901153120391.png)

![image-20210901153135162](https://uestc-fzk.gitee.io/notebook/数据库/redis/redis.assets/image-20210901153135162.png)
...
![image-20210903162024587](https://uestc-fzk.gitee.io/notebook/数据库/redis/redis.assets/image-20210903162024587.png)
...
![image-20210901000132287](https://uestc-fzk.gitee.io/notebook/数据库/redis/redis.assets/image-20210901000132287.png)
...
>
> 传统的关系型数据库里面就用到了很多这种锁机制，比如**行锁，表锁等，读锁，写锁**等，都是在**操作之前先上锁**。
>
> **乐观锁：**
>
> 乐观锁(Optimistic Lock),顾名思义，就是很乐观，每次去拿数据的时候都认为别人不会修改，所以不会上锁。
>
> 但是在更新的时候会判断一下再此期间别人有没有去更新这个数据，可以使用版本号等机制，乐观锁适用于多读的应用类型，这样可以提高吞吐量，乐观锁策略：提交版本必须大于记录当前版本才能执行更新。
...
> 刷新一个事务中已被监视的所有key。
>
> 如果执行[EXEC](http://www.redis.cn/commands/exec.html) 或者[DISCARD](http://www.redis.cn/commands/discard.html)， 则不需要手动执行[UNWATCH](http://www.redis.cn/commands/unwatch.html) 。
>

如果在 [WATCH](http://www.redis.cn/commands/watch.html) 执行之后， [EXEC](http://www.redis.cn/commands/exec.html) 执行之前， 有其他客户端修改了监视的 `key` 的值， 那么当前客户端的事务就会取消， [EXEC](http://www.redis.cn/commands/exec.html) 返回[nil-reply](http://www.redis.cn/topics/protocol.html#nil-reply)来表示事务已经失败。

而我们的程序需要做的， 就是**不断重试这个操作**， 直到没有发生碰撞为止，即直到 [EXEC](http://www.redis.cn/commands/exec.html) 的返回值不是[nil-reply](http://www.redis.cn/topics/protocol.html#nil-reply)回复即可。
...
![image-20210904182548721](https://uestc-fzk.gitee.io/notebook/数据库/redis/redis.assets/image-20210904182548721.png)

![image-20210904182632674](https://uestc-fzk.gitee.io/notebook/数据库/redis/redis.assets/image-20210904182632674.png)
...
![image-20210906170900640](https://uestc-fzk.gitee.io/notebook/数据库/redis/redis.assets/image-20210906170900640.png)
...
![image-20210906173951978](https://uestc-fzk.gitee.io/notebook/数据库/redis/redis.assets/image-20210906173951978.png)
...
![image-20210906174550752](https://uestc-fzk.gitee.io/notebook/数据库/redis/redis.assets/image-20210906174550752.png)
...
![image-20210906174959303](https://uestc-fzk.gitee.io/notebook/数据库/redis/redis.assets/image-20210906174959303.png)

缓存失效瞬间

![image-20210906175009089](https://uestc-fzk.gitee.io/notebook/数据库/redis/redis.assets/image-20210906175009089.png)
...
![image-20210906213429215](https://uestc-fzk.gitee.io/notebook/数据库/redis/redis.assets/image-20210906213429215.png)

![image-20210906213449676](https://uestc-fzk.gitee.io/notebook/数据库/redis/redis.assets/image-20210906213449676.png)
...
![image-20210908160955660](https://uestc-fzk.gitee.io/notebook/数据库/redis/redis.assets/image-20210908160955660.png)
...
![image-20210908160628588](https://uestc-fzk.gitee.io/notebook/数据库/redis/redis.assets/image-20210908160628588.png)

**LettuceConnectionFactory**

放了一个LettucePool，感觉和上面这个差不太多了。

![image-20210908162155959](https://uestc-fzk.gitee.io/notebook/数据库/redis/redis.assets/image-20210908162155959.png)
...
还需要设置密码，在redis.conf中设置：`requirepass your_password`   
...
- RDB持久化方式能够在指定的时间间隔能对你的数据进行**快照存储**.
...
- RDB 需要**经常fork子进程**来保存数据集到硬盘上；当数据集比较大的时候,fork的过程是非常耗时的

**快照**

在默认情况下， Redis 将数据库快照保存在名字为 dump.rdb的二进制文件中。

![批注 2021-09-03 234531](https://uestc-fzk.gitee.io/notebook/数据库/redis/redis.assets/批注 2021-09-03 234531.png)
...
- AOF文件：只进行追加的日志文件
- AOF 文件体积变得过大时：自动地在后台对 AOF 进行**重写**： 重写后的新 AOF 文件包含了恢复当前数据集所需的最小命令集合。 
...
```shell
appendonly no	# 是否开启append only模式作为持久化方式，默认使用的是rdb方式持久化，这 种方式在许多应用中已经足够了
...
# appendfsync no	
	# appendfsync aof持久化策略的配置 
	# no表示不执行fsync，由操作系统保证数据同步到磁盘，速度最快。 
	# always表示每次写入都执行fsync，以保证数据同步到磁盘。非常慢，也非常安全
...
4、只做缓存，如果你只希望你的数据在服务器运行的时候存在，你也可以不使用任何持久化
...
当 slave 连接到 master 时，它们使用` PSYNC `命令来发送它们记录的旧的` master replication ID `和它们至今为止处理的偏移量。
通过这种方式， master 能够仅发送 slave 所需的增量部分。但是如果 master 的缓冲区中没有足够的命令积压缓冲记录，或者如果 slave 引用了不再知道的历史记录（replication ID），则会转而进行一个**全量重同步**：在这种情况下， slave 会得到一个完整的数据集副本，从头开始。
...
![image-20210904211156227](https://uestc-fzk.gitee.io/notebook/数据库/redis/redis.assets/image-20210904211156227.png)
...
![image-20210904212530791](https://uestc-fzk.gitee.io/notebook/数据库/redis/redis.assets/image-20210904212530791.png)
...
![image-20210904213557821](https://uestc-fzk.gitee.io/notebook/数据库/redis/redis.assets/image-20210904213557821.png)

3、根据3个不同的配置文件，启动3个redis服务

![image-20210904213758343](https://uestc-fzk.gitee.io/notebook/数据库/redis/redis.assets/image-20210904213758343.png)
...
![redis主从复制](https://uestc-fzk.gitee.io/notebook/数据库/redis/redis.assets/redis主从复制.png)

不过，命令方式重启之后就无效了，真实的配置应该是在配置文件中进行配置，那样是永久的。

![image-20210904215640862](https://uestc-fzk.gitee.io/notebook/数据库/redis/redis.assets/image-20210904215640862.png)

主机可以写，可以读，从机只能读哦。

![image-20210904220140621](https://uestc-fzk.gitee.io/notebook/数据库/redis/redis.assets/image-20210904220140621.png)

测试1：主机挂了，从机查看信息；主机恢复，从机查看信息：

![image-20210904220557403](https://uestc-fzk.gitee.io/notebook/数据库/redis/redis.assets/image-20210904220557403.png)

测试2：从机挂了，查看主机信息；从机恢复，查看从机信息：

![image-20210904221335065](https://uestc-fzk.gitee.io/notebook/数据库/redis/redis.assets/image-20210904221335065.png)

#### 层层链路

上一个Slave 可以是下一个slave 和 Master，Slave 同样可以接收其他 slaves 的连接和同步请求，那么该 slave 作为了链条中下一个的master，可以有效减轻 master 的写压力

![image-20210904221913374](https://uestc-fzk.gitee.io/notebook/数据库/redis/redis.assets/image-20210904221913374.png)

![image-20210904222209367](https://uestc-fzk.gitee.io/notebook/数据库/redis/redis.assets/image-20210904222209367.png)

在原始主机上设置之后，从机都能取到；但是第二台主机依旧不能写。

![image-20210904222353541](https://uestc-fzk.gitee.io/notebook/数据库/redis/redis.assets/image-20210904222353541.png)
...
![image-20210906102156434](https://uestc-fzk.gitee.io/notebook/数据库/redis/redis.assets/image-20210906102156434.png)
...
![image-20210906151622737](https://uestc-fzk.gitee.io/notebook/数据库/redis/redis.assets/image-20210906151622737.png)

![image-20210906151641510](https://uestc-fzk.gitee.io/notebook/数据库/redis/redis.assets/image-20210906151641510.png)

![image-20210906151658034](https://uestc-fzk.gitee.io/notebook/数据库/redis/redis.assets/image-20210906151658034.png)
...
![image-20210904223939601](https://uestc-fzk.gitee.io/notebook/数据库/redis/redis.assets/image-20210904223939601.png)
...
![image-20210904224055556](https://uestc-fzk.gitee.io/notebook/数据库/redis/redis.assets/image-20210904224055556.png)
...
![image-20210904230232296](https://uestc-fzk.gitee.io/notebook/数据库/redis/redis.assets/image-20210904230232296.png)
...
![redis-哨兵模式](https://uestc-fzk.gitee.io/notebook/数据库/redis/redis.assets/redis-哨兵模式.png)
...
那么就去把requirepass给注释掉(关于bind接下来再细说)，重新启动集群：![image-20210911105345318](https://uestc-fzk.gitee.io/notebook/数据库/redis/redis.assets/image-20210911105345318.png)

现在，master：6379,6389,6390；replica：6380,6381,6391.
关闭其中的6390：6390的slave是6380
![image-20210911105529326](https://uestc-fzk.gitee.io/notebook/数据库/redis/redis.assets/image-20210911105529326.png)

可以看到6390已经fail了，6380已经成为master了。如果此时再激活6390的话：
![image-20210911105724989](https://uestc-fzk.gitee.io/notebook/数据库/redis/redis.assets/image-20210911105724989.png)
